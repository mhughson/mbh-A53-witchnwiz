...
def get_cblocks_from_bytes(block_iter, mask_iter=None, number_of_blocks=float("inf"), use_bit_flip=True, allow_partial=False):
    block_iter = iter(block_iter)
    if not mask_iter:
        mask_iter = iter(ff())
    else:
        mask_iter = iter(FileIterContextHack(mask_iter, 'rb'))
    total_blocks = 0
    while total_blocks < number_of_blocks:
        block = bytes(i for _, i in zip(range(64), block_iter))
        block_len = len(block)
        mask = bytes(i for _, i in zip(range(block_len), mask_iter))
        if block_len == 0:
            break
        if block_len < 64:
            if not allow_partial:
                ...
                mask = mask + b'\x00'*(64-block_len)
        yield compress_single_block(block, mask, use_bit_flip)
        total_blocks += 1
        ...
        for fn in options.input:
            with FileIterContextHack(fn, 'rb') as input_file:
                if options.decompress:
                    ...
                    page = []
                    for block in get_cblocks_from_bytes(input_file, options.mask, use_bit_flip=(not options.no_bit_flip), allow_partial=True):
                        page.append(block)
                        if len(page) >= 128:
                            output_file.write(b''.join(page))
                            page.clear()
